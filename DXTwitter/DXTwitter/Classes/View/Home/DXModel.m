...
@implementation DXCard
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{ @"users" : [DXUser class] };
}
+ (NSDictionary *)modelCustomPropertyMapper {
...
@implementation DXTweet
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{
             @"medias" : [DXMedia class],
             @"extendedMedias" : [DXMedia class],
             @"userMentions" : [DXUserMention class],
             @"urls" : [DXURL class],
             @"hashTags" : [DXHashTag class]
             };
}
+ (NSDictionary *)modelCustomPropertyMapper {
    return @{
...
@implementation DXAPIRespose
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{
             @"users" : [DXUser class],
             @"tweets" : [DXTweet class],
...
- (void)_updateTweetReference:(DXTweet *)tweet {
    if (!tweet) return;
    DXUser *user;
    for (DXUserMention *mention in tweet.userMentions) {
        user = mention.uidStr ? _users[mention.uidStr] : nil;
        mention.user = user;
    }
    for (DXUserMention *mention in tweet.retweetedStatus.userMentions) {
        user = mention.uidStr ? _users[mention.uidStr] : nil;
        mention.user = user;
    }
