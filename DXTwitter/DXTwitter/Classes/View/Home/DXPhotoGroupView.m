...
- (id)copyWithZone:(NSZone *)zone {
    DXPhotoGroupItem *item = [self.class new];
    return item;
}
...
@interface DXPhotoGroupCell : UIScrollView <UIScrollViewDelegate>
...
@property (nonatomic, strong) DXPhotoGroupItem *item;
@property (nonatomic, readonly) BOOL itemDidLoad;
- (void)resizeSubviewSize;
@end

@implementation DXPhotoGroupCell
...
- (void)setItem:(DXPhotoGroupItem *)item {
...
@interface DXPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation DXPhotoGroupView
...
- (void)presentFromImageView:(UIView *)fromView toContainer:(UIView *)toContainer animated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((DXPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    DXPhotoGroupCell *cell = [self cellForPage:self.currentPage];
    DXPhotoGroupItem *item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    DXPhotoGroupCell *cell = [self cellForPage:currentPage];
    DXPhotoGroupItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(DXPhotoGroupCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) {
        if (i >= 0 && i < self.groupItems.count) {
            DXPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                DXPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (DXPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (DXPhotoGroupCell *)dequeueReusableCell {
    DXPhotoGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (DXPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (DXPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    DXPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
